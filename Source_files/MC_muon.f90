...
! in 2025
! 1111111111111111111111111111111111111111111111111111111111111
! Module contains Monte Carlo routines to simulate muon events
module MC_muon
...
use MC_general_tools, only: flight_time_to_boundary, out_of_simulation_box, particle_cross_box_boundary, cos_theta_from_W, &
                            ...
                            get_muon_flight_time, remove_particle_from_MC_array, extend_MC_array, find_the_target, &
...
use CS_muon_inelastic, only: get_inelastic_energy_transfer_muon
use CS_muon_elastic, only: get_muon_elastic_CS
...
use CS_muon_Bremsstrahlung, only: get_energy_transfer_Bremsstrahlung_muon
...
subroutine MC_muon_event(used_target, numpar, N_p, Prtcl, NOP, MC, MD_supce, E_mu_at, E_e, E_h)
   type(Matter), intent(in) :: used_target   ! parameters of the target
   type(Num_par), intent(in) :: numpar   ! all numerical parameters
   integer, intent(inout) :: N_p   ! number of muons
   type(Muon), dimension(:), allocatable, intent(inout) :: Prtcl  ! the muon to perform some event
   ...
   real(8), dimension(:,:,:), intent(inout) :: E_mu_at, E_e, E_h ! data to pass to MD later
   !----------------------------------
   ...
   ! 2) Choose a muon event:
   call find_type_of_muon_event(used_target, numpar, Prtcl(NOP), i_type)  ! below
   
   ! 3) Perform the event according to the chosen type:
   select case(i_type)
   case (0) ! target boundary crossing
      call event_muon_target_boundary(used_target, numpar, Prtcl(NOP))   ! below
      
   case (1) ! inelastic
      call event_muon_inelastic(used_target, numpar, MC, NOP, MD_supce, E_e, E_h)   ! below
      
   case (2) ! elastic
      call event_muon_elastic(used_target, numpar, MC, NOP, MD_supce, E_mu_at)   ! below
      
   case (3) ! Bremsstrahlung
      call event_muon_Bremsstrahlung(used_target, numpar, MC, NOP)   ! below
      
   case(4)  ! pair creation
      ! NOT READY
   ...
   endselect
end subroutine MC_muon_event



!ССССССССССССССССССССССССССССССССССССССССССС
! muon crossing target boundary:
subroutine event_muon_target_boundary(used_target, numpar, Prtcl)
   ...
   ! Find whether an muon crosses the boundary or reflects back:
   ! 1) Kinetic energy towards crossing the boundary:
   call define_normal_to_surface(used_target,  Prtcl, norm_to_surf, 'event_muon_target_boundary')    ! module "MC_general_tools"
   ...
   Ekin_norm = kinetic_energy_from_velosity(Vnorm, g_M_muon)    ! module "Relativity"
   ...
   nullify(Em_Barr)
end subroutine event_muon_target_boundary


!BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB
! Muon Bremsstrahlung emission subroutines:
subroutine event_muon_Bremsstrahlung(used_target, numpar, MC, NOP)
   type(Matter), intent(in), target :: used_target   ! parameters of the target
   type(Num_par), intent(in) :: numpar   ! all numerical parameters
   integer, intent(in) :: NOP   ! index of particle in the array
   type(MC_arrays), intent(inout), target :: MC      ! elements of MC array for all particles in one iteration
   !------------------------------------------------------
   type(Muon), pointer :: Prtcl  ! the muon to perform some event
   ...
   Prtcl => MC%MC_Muons(NOP)
   matter => used_target%Material(Prtcl%in_target)
   
   ! 1) Select the element the muon scatters on:
   ! Get the total cross section:
   call interpolate_data_single(matter%Muon_Brems_total%E(:), matter%Muon_Brems_total%Total(:), Prtcl%Ekin, CS_tot) ! module "Little_subroutines"
   ...
   if (KOA > 1) then    ! find element
      ...
      SRCH:do j =1, matter%N_Elements-1     ! for each element, expect for the last one
         ...
         call interpolate_data_single(Element%Muon_Brems%E,  Element%Muon_Brems%Total(:), Prtcl%Ekin, CS_cur) ! module "Little_subroutines"
...
!          if (found_shl) exit SRCH  ! no need to continue the search if we already found the KOA
   ...
   if ( (KOA > size(matter%Elements(:))) .or. (KOA < 1) ) then
      print*, "Error 1 in event_muon_Bremsstrahlung:", CS_sampled, RN*CS_tot, CS_tot
   endif
   if (isnan(CS_elem) .or. (CS_elem <= 0.0d0)) then
      print*, "Error 2 in event_muon_Bremsstrahlung:", Prtcl%Ekin, CS_elem
   endif
   
   ! Now we know which element the muon scatters on:
   Element => matter%Elements(KOA)  ! all information about this element
   
   ! 2) Get the energy of emitted photon:
   dE = get_energy_transfer_Bremsstrahlung_muon(Prtcl%Ekin, Element, numpar, KOA, CS_elem)    ! module "CS_muon_Bremsstrahlung"
   
   ! 3) Get the cosines of the muon velosity:
   Vtot = sqrt( SUM( Prtcl%V(:)*Prtcl%V(:) ) )
   a0(:) = Prtcl%V(:)/Vtot
   Vtot0 = Vtot ! save it for later setting photon parameters
   
   ! 4) Get the muon deflection angles (assume the same as for electron):
   call sample_Bremsstrahlung_electron_angles(phi,theta)    ! module "CS_electrons_Bremsstrahlung"
   
   ! 5a) Get the deflection angle (angles of scattering of muon in the laboratory system):
!    call  deflect_velosity(a0(1), a0(2), a0(3), phi, theta, a(1), a(2), a(3))  ! module "MC_general_tools"
   ...
   ! 5b) Change muon energy accordingly:
   Prtcl%Ekin = Prtcl%Ekin - dE ! [eV]
   ! 5c) Change the velosity accordingly:
   Vtot = velosity_from_kinetic_energy(Prtcl%Ekin, g_M_muon)  ! [A/fs] module "Relativity"
   ! Update velosity:
   Prtcl%V(:) = Vtot * a(:)  ! components of the muon velosity
   
   ! 6) Now update the muon parameters:
   call get_muon_flight_time(used_target, numpar, Prtcl)  ! module "MC_general_tools"
   ...
   ! 7b) Get the cosines of the emitting muon velosity:
...
end subroutine event_muon_Bremsstrahlung


!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
! Muon inelastic scattering subroutines:
subroutine event_muon_inelastic(used_target, numpar, MC, NOP, MD_supce, E_e, E_h)
   type(Matter), intent(in), target :: used_target   ! parameters of the target
   type(Num_par), intent(in) :: numpar   ! all numerical parameters
   integer, intent(in) :: NOP   ! index of particle in the array
   type(MC_arrays), intent(inout), target :: MC      ! elements of MC array for all particles in one iteration
...
!    type(Atom_kind), pointer :: Element
   ...
   ! Sample the transfered energy:
    if (KOA == 0) then   ! valence band:
      valent = .true.
      dE = get_inelastic_energy_transfer_muon(Prtcl%Ekin, matter, matter%DOS, numpar, KOA, NSH, matter%DOS%Egap)    ! module "CS_muon_inelastic"
    else ! core shell:
      valent = .false.
      dE = get_inelastic_energy_transfer_muon(Prtcl%Ekin, matter, matter%DOS, numpar, KOA, NSH, matter%Elements(KOA)%Ip(NSH))    ! module "CS_muon_inelastic"
   endif
   ...
   call sample_angles_inelastic_muon(Prtcl%Ekin, dE, theta, phi) ! below
   ! And the angles of emission of a new electron:
   call set_angles_of_new_electron_muon(Prtcl%Ekin, dE, phi, theta_r, phi_r)  ! below
   ...
   ! 4) Change the incident muon direction of motion:
   ! Get the cosines of the muon velosity:
   ...
   ! Get the NEW absolute muon velosity, to set its components according to the cosines:
   Prtcl%Ekin = Prtcl%Ekin - dE ! update muon energy
   V_e_abs = velosity_from_kinetic_energy(Prtcl%Ekin, g_M_muon)     ! [A/fs] module "Relativity"
   V_e(:) = V_e_abs * V(:)  ! components of the muon velosity
   ! Update velosity:
   Prtcl%V(:) = V_e(:)
   
   ! Get the muon next sampled free path:
   call get_muon_flight_time(used_target, numpar, Prtcl)  ! module "MC_general_tools"
...
end subroutine event_muon_inelastic
...
subroutine select_shell_ionization_pos(used_target, numpar, MC, NOP, KOA, NSH)
   ...
   !------------------------------------------------------
   type(Target_atoms), pointer :: matter
   type(Muon), pointer :: Prtcl  ! the photon to perform some event
   type(Atom_kind), pointer :: Element
   ...
   ! Get the total cross section:
   call interpolate_data_single(matter%Muon_inelastic_total%E(:), matter%Muon_inelastic_total%Total(:), Prtcl%Ekin, CS_tot) ! module "Little_subroutines"
   ! Get the random number, to sample partial cross section:
   call random_number(RN)   ! intrinsic FORTRAN subroutine
   ! Realized CS:
   CS_sampled = RN*CS_tot
      ...
      do k = 1, Element%N_shl    ! for all shells in this elements
         ...
         VAL:if ((numpar%El_inelast /= 2) .and.  (Element%valent(k)) .and. (allocated(matter%CDF_valence%A)) ) then    ! Valence band (not for RBEB atomic model!)
            if (.not.valence_done) then ! add VB (but only once):
               call interpolate_data_single(matter%Muon_inelastic_valent%E,  matter%Muon_inelastic_valent%Total(:), Prtcl%Ekin, CS_cur) ! module "Little_subroutines"
               CS_sum = CS_sum + CS_cur  ! band contribution
               valence_done = .true.     ! we added VB once (not do double-count it)
            endif
         else VAL
            call interpolate_data_single(Element%Muon_inelastic%E,  Element%Muon_inelastic%Per_shell(k,:), Prtcl%Ekin, CS_cur) ! module "Little_subroutines"
...
subroutine sample_angles_inelastic_muon(Ekin, dE, theta, phi)
   ...
   mu = cos_theta_from_W(Ekin, dE, g_M_muon, g_me)    ! module "MC_general_tools"
   theta = acos(mu)
end subroutine sample_angles_inelastic_muon


pure subroutine set_angles_of_new_electron_muon(Ekin, dE, phi, theta_r, phi_r)
   real(8), intent(in) :: Ekin, dE  ! [eV] incident and transfered energy
   ...
   mu = cos_recoil_from_W(Ekin, dE, g_M_muon, g_me)    ! module "MC_general_tools"
   theta_r = acos(mu)
end subroutine set_angles_of_new_electron_muon


!EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE
! Elastic scattering energy transfer
subroutine event_muon_elastic(used_target, numpar, MC, NOP, MD_supce, E_mu_at)
   type(Matter), intent(in), target :: used_target   ! parameters of the target
   type(Num_par), intent(in) :: numpar   ! all numerical parameters
   integer, intent(in) :: NOP   ! index of particle in the array
   type(MC_arrays), intent(inout), target :: MC      ! elements of MC array for all particles in one iteration
   type(MD_supcell), intent(in) :: MD_supce  ! MD supercell parameters for connection between MC and MD modules
   real(8), dimension(:,:,:), intent(inout) :: E_mu_at ! data to pass to MD later
   !------------------------------------------------------
   type(Muon), pointer :: Prtcl  ! the muon to perform some event
   type(Target_atoms), pointer :: matter
   type(Atom_kind), pointer :: Element
   ...
   call elastic_energy_transfer_mu(numpar, Prtcl, matter, KOA, dE, theta)   ! below
   ! Just in case, check that the transfered energy is not too high:
   if (dE > Prtcl%Ekin) dE = Prtcl%Ekin ! [eV]
   
   ! 4) Get the cosines of the muon velosity:
   Vtot = sqrt( SUM( Prtcl%V(:)*Prtcl%V(:) ) )
   a0(:) = Prtcl%V(:)/Vtot
...
!    call deflect_velosity(a0(1), a0(2), a0(3), phi, theta, a(1), a(2), a(3))  ! module "MC_general_tools"
   ...
   if (numpar%DO_MD) then   ! if user requested MD at all
      call add_energy_into_MD_array(numpar, dE, Prtcl%R, MD_supce, E_mu_at)   ! module "MC_general_tools"
   endif

   nullify(Prtcl, matter, Element)
end subroutine event_muon_elastic



subroutine elastic_energy_transfer_mu(numpar, Prtcl, matter, KOA, dE, theta)
   type(Num_par), intent(in) :: numpar   ! all numerical parameters
   type(Muon), intent(in) :: Prtcl  ! the electron to perform some event
...
!       E_right = matter%E_debye    ! [eV] maximal transferred energy
      ...
      Mc2 = rest_energy(g_M_muon)   ! incident muon rest energy; module "Relativity"
      E_right = W_max(Mc2, mtc2, .false., Prtcl%Ekin, 1.0d-8, hw_phonon) ! module "CS_integration_limits"
      
      Mu_elastic = numpar%Mu_elastic
      if (Mu_elastic == 1) then
         max_E0 = maxval(matter%CDF_phonon%E0(:))
         call find_Wmax_equal_Wmin(g_M_muon, matter%Mean_Mass, .false., Prtcl%Ekin, 1.0d-6, max_E0, Eeq, hw_phonon)   ! module "CS_integration_limits"
      ...
      call get_muon_elastic_CS(Prtcl%Ekin, matter, Element, numpar, CS_tot)   ! module "CS_electrons_elastic"
      ! Sample the cross section:
      CS_sampled = RN*CS_tot
      ...
      if ( (RN < 1.0d-10) .or. (CS_sampled < 1.0d-12) ) then   ! no need to sample, it's just the lower limit
         ...
         if (Mu_elastic /= 4) then    ! for analytical CSs, use bisection search algorithm:
            ! Start finding CS:
            E_cur = (E_left + E_right)*0.5d0
            call get_muon_elastic_CS(Prtcl%Ekin, matter, Element, numpar, CS_cur, E_max=E_cur)   ! module "CS_electrons_elastic"
...
!             do while (ABS(CS_cur - CS_sampled) > eps*CS_sampled)
            do while (abs(E_left - E_right) >= eps*E_left)
               ...
               call get_muon_elastic_CS(Prtcl%Ekin, matter, Element, numpar, CS_cur, E_max=E_cur)   ! module "CS_electrons_elastic"
            enddo
!             print*, 'elastic_energy_transfer_mu 1', Mu_elastic, CS_sampled, CS_cur, E_cur
         else   ! for numerically integrable CS, use direct search
             ! Target mean atomic number:
             Zeff = 1.0d0 + Equilibrium_charge_SHI(Prtcl%Ekin, g_M_muon, matter%Mean_Z, (matter%Mean_Z-1.0d0), 0, 1.0d0) ! module "SHI_charge_state"
             call CDF_total_CS_nonrel(numpar, CS_cur, Se1, Prtcl%Ekin, g_M_muon, Zeff, 1.0d-10, matter%T_eV, matter%CDF_phonon, matter%Mean_Mass, &
...
!              print*, 'elastic_energy_transfer_mu 2', Mu_elastic, CS_sampled, CS_cur, E_cur
         endif  ! (El_elast /= 4)
      ...
      mu = cos_theta_from_W(Prtcl%Ekin, dE, g_M_muon, matter%Mean_Mass) ! module "MC_general_tools"
!       print*, 'mu', mu, Prtcl%Ekin, dE, E_left, CS_sampled, CS_tot
   ...
   case (2)  ! Mott
      ...
      ! Get the total cross section:
      call interpolate_data_single(matter%Muon_elastic_total%E(:), matter%Muon_elastic_total%Total(:), Prtcl%Ekin, CS_tot) ! module "Little_subroutines"
      ! Get the random number, to sample partial cross section:
      call random_number(RN)   ! intrinsic FORTRAN subroutine
      ! Realized CS:
      CS_sampled = RN*CS_tot
      ...
      SRCH:do j =1, matter%N_Elements-1     ! for each element, expect for the last one
         ...
         elem_contrib = dble(Element%percentage)/N_elem   ! element contribution to this compound (e.g. in SiO2: it is 1/3 of Si, 2/3 of O)
         ! For each shell, find its partial cross section:
         call interpolate_data_single(Element%Muon_elastic%E,  Element%Muon_elastic%Total(:), Prtcl%Ekin, CS_cur) ! module "Little_subroutines"
         CS_sum = CS_sum + CS_cur*elem_contrib
         ! Check if this shell is the sampled one according to the integral CS:
...
!          if (found_at) exit SRCH  ! no need to continue the search if we already found the KOA
      ...
      call  get_muon_elastic_polar_angle(Prtcl%Ekin, Element, theta) ! below
         
      ! 3) Change electron energy accordingly:
      dE = transfered_E_from_theta(Prtcl%Ekin, theta, g_M_muon, Element%M) ! module "MC_general_tools"
   ...
   nullify(Element)
end subroutine elastic_energy_transfer_mu


subroutine get_muon_elastic_polar_angle(Ekin, Element, theta)
   real(8), intent(in) :: Ekin  ! [eV] muon energy
...
end subroutine get_muon_elastic_polar_angle


!TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT

subroutine find_type_of_muon_event(used_target, numpar, Prtcl, i_type)
   type(Matter), intent(in), target :: used_target   ! parameters of the target
   type(Num_par), intent(in) :: numpar   ! all numerical parameters
   type(Muon), intent(in) :: Prtcl  ! the muon to perform some event
...
!        print*, 'Pos2', Prtcl%Ekin, Prtcl%ti, Prtcl%t_sc, Prtcl%R(:)
      ...
      call find_type_of_scattering(i_type, Prtcl%Ekin,  matter%Muon_inelastic_total%E, matter%Muon_inelastic_total%Total, &
   ...
   nullify(matter)
end subroutine find_type_of_muon_event
