...
module CS_positrons_Bremsstrahlung
use Universal_constants
use CDF_Ritchi
use Relativity
use Objects
use Read_input_data, only: m_positron_CS, m_input_folder, m_positron_Brems_CS, m_folder_materials
...
subroutine get_positron_Brems(Material, numpar, Err)
   ...
   TRGT:do i = 1, N_targets	! for each target
      ...
      LMNT:do j =1, N_elements	! for each element
         ...
         !-------------------------------------
         ! Set part of the file name corresponding to the model used for positron Bremsstrahlung CS:
         ...
         allocate(Element%Pos_brems%E(Ngrid))
         Element%Pos_brems%E = Element%Phot_absorption%E
         allocate(Element%Pos_brems%Total(Ngrid))
         allocate(Element%Pos_brems%Total_MFP(Ngrid))
...
!          if (file_exist) then	! only create it if file does not exist
         if ((file_exist) .and. (.not.numpar%recalculate_MFPs)) then    ! just read from the file, no need to recalculate:
            ...
            do m = 1, Ngrid	! for all energy grid points:
               read(FN,'(es,es,es)', IOSTAT=Reason) Element%Pos_brems%E(m), Element%Pos_brems%Total(m), Element%Pos_brems%Total_MFP(m) 
...
8392     open(newunit = FN, FILE = trim(adjustl(File_name)),action='write')
            do m = 1, Ngrid	! for all energy grid points:
               ...
               write(FN,'(es,es,es)') Element%Pos_brems%E(m), Element%Pos_brems%Total(m), Element%Pos_brems%Total_MFP(m) 
            enddo ! m = 1, Ngrid
         endif
         close(FN)
         ...
         Element%Pos_brems%Total_MFP(:) = Element%Pos_brems%Total_MFP(:) * 1.0d24/(Material(i)%At_Dens * elem_contrib)
      enddo LMNT
      ...
      File_name = trim(adjustl(Path_total))//path_sep//trim(adjustl(m_positron_Brems_CS))//'_total_'//trim(adjustl(Model_name))//'.dat'
      ...
      allocate(Material(i)%Pos_Brems_total%E(Nsiz))
      Material(i)%Pos_Brems_total%E = Material(i)%Ph_absorption_total%E
      allocate(Material(i)%Pos_Brems_total%Total(Nsiz))
      allocate(Material(i)%Pos_Brems_total%Total_MFP(Nsiz))
      Material(i)%Pos_Brems_total%Total(:) = 0.0d0
      Material(i)%Pos_Brems_total%Total_MFP(:) = 0.0d0
      ...
      do j =1, N_elements	! for each element
         ...
         do m = 1, Nsiz	! for all energy grid points:
            E => Material(i)%Pos_Brems_total%E(m)    ! photon energy [eV]
            call interpolate_data_single(Element%Pos_brems%E,  Element%Pos_brems%Total(:), E, sigma) ! module "Little_subroutines"
            call interpolate_data_single(Element%Pos_brems%E,  Element%Pos_brems%Total_MFP(:), E, MFP) ! module "Little_subroutines"
            ! Add them into the arrays:
            Material(i)%Pos_Brems_total%Total(m) = Material(i)%Pos_Brems_total%Total(m) + sigma*elem_contrib
            Material(i)%Pos_Brems_total%Total_MFP(m) = Material(i)%Pos_Brems_total%Total_MFP(m) + 1.0d0/MFP !*elem_contrib ! to be inverted
         enddo ! m
      enddo ! j
      ! Invert to get MFP:
      Material(i)%Pos_Brems_total%Total_MFP(:) = 1.0d0/Material(i)%Pos_Brems_total%Total_MFP(:) ! [A]
      ...
      if (.not.file_exist .or. numpar%recalculate_MFPs) then	! only create it if file does not exist
         ...
         do m = 1, Nsiz	! for all energy grid points:
            write(FN,'(es,es,es)') Material(i)%Pos_Brems_total%E(m), Material(i)%Pos_Brems_total%Total(m), Material(i)%Pos_Brems_total%Total_MFP(m)
         enddo ! m = 1, Nsiz
         close(FN)
      endif
...
end subroutine get_positron_Brems


subroutine get_pos_Brems_CS(Ee, Element, numpar, sigma, E_max_in)
   real(8), intent(in) :: Ee	! [eV] positron energy
   ...
   real(8) :: Fp
   select case (numpar%Pos_Brems)	! Bremsstrahlung scattering: 0=excluded, 1=BHW
   case (1)	! BHW
      ...
      ! Add the positron correction to the electron CS, Eq.(58) [2]:
      Fp = positron_Brems_correction(dble(Element%Zat), Ee, g_me_eV)  ! below
      sigma = Fp * sigma
   case default	! exclude
      sigma = 0.0d0
   end select
end subroutine get_pos_Brems_CS


pure function positron_Brems_correction(Z, Ee, mc2) result(Fp)
...
end function positron_Brems_correction



function get_energy_transfer_Bremsstrahlung_pos(Ee, Element, numpar, j, CS_tot) result(dE)
...
!    CS_cur = Bremsstrahlung_total_CS(Ee, Element, E_cur)  ! module "CS_electrons_Bremsstrahlung"
   call get_pos_Brems_CS(Ee, Element, numpar, CS_cur, E_cur)    ! above
...
!       CS_cur =  Bremsstrahlung_total_CS(Ee, Element, E_cur) ! module "CS_electrons_Bremsstrahlung"
   ...
   dE = E_cur
end function get_energy_transfer_Bremsstrahlung_pos


end module CS_positrons_Bremsstrahlung
