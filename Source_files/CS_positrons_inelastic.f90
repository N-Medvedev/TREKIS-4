...
! in 2019
! 1111111111111111111111111111111111111111111111111111111111111
! Module contains all cross sections for positroninelastic scattering 
! References used:
...
module CS_positrons_inelastic
use Universal_constants
use CDF_delta, only : energy_loss_delta, CDF_total_CS_delta
use Objects
use Read_input_data, only: m_input_folder, m_folder_materials, m_positron_CS, m_positron_inelast_CS
...
subroutine get_positron_IMFP(Material, numpar, Err)
   ...
   write(*, '(a)', advance='no') ' Obtaining positron inelastic scattering cross sections...'
   ...
   TRGT:do i = 1, N_targets	! for each target
      ...
      LMNT:do j =1, N_elements	! for each element
         ...
         allocate(Element%Pos_inelastic%E(Ngrid))
         Element%Pos_inelastic%E = Element%Phot_absorption%E
         allocate(Element%Pos_inelastic%Total(Ngrid))
         allocate(Element%Pos_inelastic%Total_MFP(Ngrid))
         allocate(Element%Pos_inelastic%Total_Se(Ngrid))
         allocate(Element%Pos_inelastic%Per_shell(Element%N_shl,Ngrid))
         allocate(Element%Pos_inelastic%MFP(Element%N_shl,Ngrid))
         allocate(Element%Pos_inelastic%Se(Element%N_shl,Ngrid))
         ...
         do k = 1, N_shells
            ...
            VAL:if ((numpar%Pos_inelast /= 2) .and.  (Element%valent(k)) .and. (allocated(Material(i)%CDF_valence%A)) ) then    ! Valence band (not for RBEB atomic model!)
               if (.not.allocated(Material(i)%Pos_inelastic_valent%E)) then   ! valence band has not been defined yet, so do that
                  Nsiz = size(Material(i)%Ph_absorption_valent%E)   ! valence band has a different grid from core shells
                  allocate(Material(i)%Pos_inelastic_valent%E(Nsiz))
                  Material(i)%Pos_inelastic_valent%E = Material(i)%Ph_absorption_valent%E    ! we already set this grid for photons, reuse it
                  allocate(Material(i)%Pos_inelastic_valent%Total(Nsiz))
                  allocate(Material(i)%Pos_inelastic_valent%Total_MFP(Nsiz))
                  allocate(Material(i)%Pos_inelastic_valent%Total_Se(Nsiz))
                  !allocate(Material(i)%Pos_inelastic_valent%Per_shell(Element%N_shl,Nsiz)) ! --- leave it not allocated !!!
                  ! Check file with CSs:
                  !File_name = trim(adjustl(Folder_with_CS))//path_sep//trim(adjustl(m_positron_inelast_CS))//'_valence_'//trim(adjustl(Model_name))//'.dat'
                  File_name = trim(adjustl(Path_valent))//path_sep//trim(adjustl(m_positron_inelast_CS))//'_valence_'//trim(adjustl(Model_name))//'.dat'
...
!                   if (file_exist) then	! just read from the file, no need to recalculate:
                  if ((file_exist) .and. (.not.numpar%recalculate_MFPs)) then    ! just read from the file, no need to recalculate:
                     ...
                     do m = 1, Nsiz	! for all energy grid points:
                        E => Material(i)%Pos_inelastic_valent%E(m)	! electron energy [eV]
                        read(FN,'(es,es,es,es)', IOSTAT=Reason) E, Material(i)%Pos_inelastic_valent%Total(m), Material(i)%Pos_inelastic_valent%Total_MFP(m), Material(i)%Pos_inelastic_valent%Total_Se(m)
                        ...
                        Material(i)%Pos_inelastic_valent%Total_MFP(m) = MFP_from_sigma(Material(i)%Pos_inelastic_valent%Total(m),  Material(i)%At_Dens) ! [A] module "CS_general_tools"
                     enddo
                  else	! no such file => create it
8391              open(newunit = FN, FILE = trim(adjustl(File_name)),action='write')
                     do m = 1, Nsiz	! for all energy grid points:
                        E => Material(i)%Pos_inelastic_valent%E(m)	! electron energy [eV]
                        !call get_pos_inelastic_CS(E, Material(i), Material(i)%DOS, numpar, j, k, Material(i)%DOS%Egap, sigma, CDF_dispers=0, CDF_m_eff=1, Se=Se)	! see below
                        call get_pos_inelastic_CS(E, Material(i), Material(i)%DOS, numpar, j, k, Material(i)%DOS%Egap, sigma, &
                                                          CDF_dispers=numpar%CDF_dispers, CDF_m_eff=numpar%CDF_m_eff, Se=Se)    ! see below
                        Material(i)%Pos_inelastic_valent%Total(m) = sigma   ! [A^2]
                        lambda = MFP_from_sigma(sigma,  Material(i)%At_Dens)    ! module "CS_general_tools"
                        Material(i)%Pos_inelastic_valent%Total_MFP(m) = lambda  ! [A]
                        Material(i)%Pos_inelastic_valent%Total_Se(m) = Se    ! [eV/A]
                        write(FN,'(es,es,es,es)') Material(i)%Pos_inelastic_valent%E(m), sigma, lambda, Se
                     enddo ! m = 1, Nsiz
                  endif ! (file_exist)
                  close(FN)
               endif ! (.not.allocated(Material(i)%Pos_inelastic_valent))
            else VAL    ! core shell
               ! Check file with CSs:
               File_name = trim(adjustl(Folder_with_CS))//path_sep//trim(adjustl(m_positron_inelast_CS))//'_'//trim(adjustl(Element%Shell_name(k)))//'_'//trim(adjustl(Model_name))//'.dat'
               inquire(file=trim(adjustl(File_name)),exist=file_exist) ! check if this file is there
!                if (file_exist) then	! just read from the file, no need to recalculate:
               if ((file_exist) .and. (.not.numpar%recalculate_MFPs)) then    ! just read from the file, no need to recalculate:
                  open(newunit = FN, FILE = trim(adjustl(File_name)),action='read')
                  ...
                  do m = 1, Ngrid	! for all energy grid points:
                     ...
                     read(FN,'(es,es,es,es)', IOSTAT=Reason) E, Element%Pos_inelastic%Per_shell(k,m), Element%Pos_inelastic%MFP(k,m), Element%Pos_inelastic%Se(k,m)
                     call read_file(Reason, count_lines, read_well)	! module "Dealing_with_files"
                     if (.not.read_well) then
                        close(FN)	! redo the file
                        goto 8390
                     endif
!                       print*, k, m, Element%Pos_inelastic%E(m), Element%Pos_inelastic%Per_shell(k,m)
                  enddo
               else	! no such file => create it
8390           open(newunit = FN, FILE = trim(adjustl(File_name)),action='write')
                  do m = 1, Ngrid	! for all energy grid points:
                     E => Element%Pos_inelastic%E(m)	! electron energy [eV]
                     !call get_pos_inelastic_CS(E, Material(i), Material(i)%DOS, numpar, j, k, Material(i)%Elements(j)%Ip(k), sigma, CDF_dispers=0, CDF_m_eff=1, Se=Se)	! see below
                     call get_pos_inelastic_CS(E, Material(i), Material(i)%DOS, numpar, j, k, Material(i)%Elements(j)%Ip(k), sigma, &
                                                          CDF_dispers=numpar%CDF_dispers, CDF_m_eff=numpar%CDF_m_eff, Se=Se)    ! see below
                     Element%Pos_inelastic%Per_shell(k,m) = sigma	! [A^2]
                     Element%Pos_inelastic%Se(k,m) = Se * 1.0d24/Material(i)%At_Dens    ! [eV/A]
                     Element%Pos_inelastic%MFP(k,m) = MFP_from_sigma(Element%Pos_inelastic%Per_shell(k,m),  1.0d24)    ! module "CS_general_tools"
                     write(FN,'(es,es,es,es)') Element%Pos_inelastic%E(m), sigma, Element%Pos_inelastic%MFP(k,m), Element%Pos_inelastic%Se(k,m)
                  enddo ! m = 1, Ngrid
               endif ! (file_exist)
               close(FN)
               ...
               Element%Pos_inelastic%MFP(k,:) = Element%Pos_inelastic%MFP(k,:) * 1.0d24/(Material(i)%At_Dens * elem_contrib)
               Element%Pos_inelastic%Se(k,:) = Element%Pos_inelastic%Se(k,:) * Material(i)%At_Dens/1.0d24 * elem_contrib
            endif VAL
         enddo ! k = 1, N_shells
      ...
      allocate(Material(i)%Pos_inelastic_total%E(Nsiz))
      Material(i)%Pos_inelastic_total%E = Material(i)%Ph_absorption_total%E
      allocate(Material(i)%Pos_inelastic_total%Total(Nsiz))
      allocate(Material(i)%Pos_inelastic_total%Total_MFP(Nsiz))
      allocate(Material(i)%Pos_inelastic_total%Total_Se(Nsiz))
      Material(i)%Pos_inelastic_total%Total(:) = 0.0d0
      Material(i)%Pos_inelastic_total%Total_MFP(:) = 0.0d0
      Material(i)%Pos_inelastic_total%Total_Se(:) = 0.0d0
      ...
      do j =1, N_elements	! for each element
         ...
         do m = 1, Nsiz	! for all energy grid points:
            E => Material(i)%Pos_inelastic_total%E(m)    ! electron energy [eV]
            ! Core shells:
            ...
            do k = 1, Element%N_shl		! for all shells in this elements
               if (.not.Element%valent(k)) then ! core orbitals
                  ...
                  call interpolate_data_single(Element%Pos_inelastic%E,  Element%Pos_inelastic%Per_shell(k,:), E, sigma_cur) ! module "Little_subroutines"
                  call interpolate_data_single(Element%Pos_inelastic%E,  Element%Pos_inelastic%Se(k,:), E, Se_cur) ! module "Little_subroutines"
                  call interpolate_data_single(Element%Pos_inelastic%E,  Element%Pos_inelastic%MFP(k,:), E, lambda_cur) ! module "Little_subroutines"
            ...
            enddo ! k
            Material(i)%Pos_inelastic_total%Total(m) = Material(i)%Pos_inelastic_total%Total(m) + sigma   ! [A^2]
            Material(i)%Pos_inelastic_total%Total_Se(m) = Material(i)%Pos_inelastic_total%Total_Se(m) +  Se   ! [eV/A]
            Material(i)%Pos_inelastic_total%Total_MFP(m) = Material(i)%Pos_inelastic_total%Total_MFP(m) + lambda  ! [1/A] to be inversed below
            
            ! Add valence band / shells:
            if ((numpar%Pos_inelast /= 2) .and. (allocated(Material(i)%CDF_valence%A)) ) then    ! valence band is defined and used in the CS model (RBEB excluded)
               if (j == 1) then ! add valence band only once - when studying the first element of the material
                  Material(i)%Pos_inelastic_total%Total(m) = Material(i)%Pos_inelastic_total%Total(m) + Material(i)%Pos_inelastic_valent%Total(m)   ! [A^2]
                  Material(i)%Pos_inelastic_total%Total_MFP(m) = Material(i)%Pos_inelastic_total%Total_MFP(m) + 1.0d0/Material(i)%Pos_inelastic_valent%Total_MFP(m)   ! [1/A]
                  Material(i)%Pos_inelastic_total%Total_Se(m) = Material(i)%Pos_inelastic_total%Total_Se(m) + Material(i)%Pos_inelastic_valent%Total_Se(m)   ! [eV/A]
               endif
      ...
      Material(i)%Pos_inelastic_total%Total_MFP = 1.0d0/Material(i)%Pos_inelastic_total%Total_MFP  ! [1/A] -> [A]
      ! Save the total cross section into the file:
      File_name = trim(adjustl(Path_valent))//path_sep//trim(adjustl(m_positron_inelast_CS))//'_total_'//trim(adjustl(Model_name))//'.dat'
      inquire(file=trim(adjustl(File_name)),exist=file_exist) ! check if this file is there
      if (.not.file_exist .or. numpar%recalculate_MFPs) then	! only create it if file does not exist
         ...
         do m = 1, Nsiz	! for all energy grid points:
            write(FN,'(es,es,es,es)') Material(i)%Pos_inelastic_total%E(m), Material(i)%Pos_inelastic_total%Total(m), &
                                                  Material(i)%Pos_inelastic_total%Total_MFP(m), Material(i)%Pos_inelastic_total%Total_Se(m)
         enddo ! m = 1, Nsiz
         close(FN)
      endif
   enddo TRGT
   ...
   write(*, '(a)') ' Done.'
!    print*, 'Positron inelastic scattering cross sections are obtained.'
   
   nullify(path_sep, E, Element)
end subroutine get_positron_IMFP




! Interface to select the model of positron inelastic scattering cross section:
function get_inelastic_energy_transfer_pos(Ee, Material, DOS, numpar, j, k, Ip, CDF_dispers, CDF_m_eff, hw_phonon) result(dE)
   ...
   real(8) :: Zeff, max_E0, Eeq, RN, CS_sampled, CS_cur, E_left, E_right, E_cur, mtc2, Se1
   real(8) :: CS_tot    ! [A^2] precalculated total cross section
   integer :: dispers, m_eff, Pos_inelast
   ...
   Zeff = 1.0d0	! positron charge
   Pos_inelast = numpar%Pos_inelast   ! to chose the model of CS calculations below
   ...
   if (Pos_inelast == 3) then
      ...
      call find_Wmax_equal_Wmin(mtc2, mtc2, .false., Ee, Ip, max_E0, Eeq)   ! module "CS_integration_limits"
...
!    E_right = (Ip + Ee)*0.5d0    ! [eV] maximal transferred energy
   E_right = W_max(mtc2, mtc2, .false., Ee, Ip)  ! module "CS_integration_limits"
   ...
   CS_tot = get_integral_inelastic_CS_pos(Ee, Material, DOS, numpar, j, k, Ip, Pos_inelast, Zeff, dispers, m_eff, E_right)   ! below
   CS_sampled = RN*CS_tot
   
   ! For analytucally integrable CDF, use bisection method:
   if (Pos_inelast /= 4) then
      ...
      CS_cur = get_integral_inelastic_CS_pos(Ee, Material, DOS, numpar, j, k, Ip, Pos_inelast, Zeff, dispers, m_eff, E_cur)   ! below
...
!       do while (ABS(CS_cur - CS_sampled)/CS_sampled > eps)
      do while (abs(E_left - E_right) >= eps1*E_left)
         ...
         CS_cur = get_integral_inelastic_CS_pos(Ee, Material, DOS, numpar, j, k, Ip, Pos_inelast, Zeff, dispers, m_eff, E_cur)   ! below
      enddo
!       print*, 'get_inelastic_energy_transfer_pos 1', Pos_inelast, CS_sampled, CS_cur, E_cur
      ...
      VAL3:if ( (k == 0) .and. (allocated(Material%CDF_valence%A)) ) then    ! Valence band
         ...
         case default  ! free electron
            call CDF_total_CS_nonrel(numpar, CS_cur, Se1, Ee, g_me, Zeff, Ip, Material%T_eV, Material%CDF_valence, g_me, &
                    DOS%k, DOS%Eff_m, .false., 1.0d0, Material%At_Dens, Material%DOS, numpar%CDF_model, &
         ...
         case (1)  ! Plasmon pole
            call CDF_total_CS_nonrel(numpar, CS_cur, Se1, Ee, g_me, Zeff, Ip, Material%T_eV, Material%CDF_valence, g_me, DOS%k, DOS%Eff_m, &
                     .false., 1.0d0, Material%At_Dens, Material%DOS, numpar%CDF_model,&
                     CDF_dispers=numpar%CDF_dispers, v_f=Material%DOS%v_f, &
                     Sigma_sampled=CS_sampled, E_sampled=E_cur)  ! module "CS_electrons_inelastic"
         case (2)  ! Ritchie extended
            call CDF_total_CS_nonrel(numpar, CS_cur, Se1, Ee, g_me, Zeff, Ip, Material%T_eV, Material%CDF_valence, g_me, DOS%k, DOS%Eff_m, &
                     .false., 1.0d0, Material%At_Dens, Material%DOS, numpar%CDF_model, CDF_dispers=numpar%CDF_dispers, &
                     Sigma_sampled=CS_sampled, E_sampled=E_cur)  ! module "CS_electrons_inelastic"
         ...
         case (1)  ! Plasmon pole
            call CDF_total_CS_nonrel(numpar, CS_cur, Se1, Ee, g_me, Zeff, Ip, Material%T_eV, Material%Elements(j)%CDF(k), g_me, DOS%k, DOS%Eff_m, &
                     .false.,1.0d0, Material%At_Dens, Material%DOS, numpar%CDF_model, CDF_dispers=numpar%CDF_dispers, v_f=Material%DOS%v_f, &
                     Sigma_sampled=CS_sampled, E_sampled=E_cur)  ! module "CS_electrons_inelastic"
         case (2)  ! Ritchie extended
            call CDF_total_CS_nonrel(numpar, CS_cur, Se1, Ee, g_me, Zeff, Ip, Material%T_eV, Material%Elements(j)%CDF(k), g_me, DOS%k, DOS%Eff_m, &
                     .false., 1.0d0, Material%At_Dens, Material%DOS, numpar%CDF_model, CDF_dispers=numpar%CDF_dispers, &
...
!       print*, 'get_inelastic_energy_transfer_pos 2', Pos_inelast, CS_sampled, CS_cur, E_cur
   endif    ! (El_inelast /= 4)

   ! Output: sampled transferred energy:
   dE = E_cur
end function get_inelastic_energy_transfer_pos


function get_integral_inelastic_CS_pos(Ee, Material, DOS, numpar, j, k, Ip, Pos_inelast, Zeff, dispers, m_eff, Emax) result (sigma)
   ...
   integer, intent(in) :: j, k	! number or element and its shell
   real(8), intent(in) :: Ip    ! [eV] ionization potential or band gap
   integer, intent(in) :: Pos_inelast    ! model for the inelastic cross section
   ...
   select case (Pos_inelast)  ! chose which model for electron inelastic cross section to use
   case (1:3,5)	! CDF with delta-functions
      ...
      VAL2:if ( (k == 0) .and. (allocated(Material%CDF_valence%A)) ) then    ! Valence band
         sigma = CDF_total_CS_delta(Pos_inelast, Ee, g_me, Zeff, Ip, Material%At_Dens, Material%CDF_valence, g_me, .false., Emax_in = Emax)    ! module "CDF_delta"
      else VAL2  ! core shells:
         sigma = CDF_total_CS_delta(Pos_inelast, Ee, g_me, Zeff, Ip, Material%At_Dens, Material%Elements(j)%CDF(k), g_me, .false., Emax_in = Emax)    ! module "CDF_delta"
      endif VAL2
...
end function get_integral_inelastic_CS_pos




! Interface to select the model of electron inelastic scattering cross section:
 subroutine get_pos_inelastic_CS(Ee, Material, DOS, numpar, j, k, Ip, sigma, CDF_dispers, CDF_m_eff, Se)
   real(8), intent(in) :: Ee	! [eV] positron kinetic energy
   type(Target_atoms), intent(in) :: Material	!material parameters of each target that it's constructed of
   type(Density_of_states), intent(in):: DOS	! DOS of the material
   type(Num_par), intent(in) :: numpar	! all numerical parameters
   ...
   integer, intent(in), optional :: CDF_dispers, CDF_m_eff	! dispersion relation and effective mass
   !---------------------------------------
   real(8) :: Zeff, Eeq, max_E0, Se1, mtc2
   integer :: dispers, m_eff, Pos_inelast
   Zeff = 1.0d0	! electron charge
   Pos_inelast = numpar%Pos_inelast   ! to chose the model of CS calculations below
   
   ! In case it is delta-CDF model, we have to make sure the energy is not below its applicability limit:
  select case (Pos_inelast)
   case (1:3,5)	! CDF with delta-functions
      VAL0:if ( (Material%Elements(j)%valent(k)) .and. (allocated(Material%CDF_valence%A)) ) then    ! Valence band
         max_E0 = maxval(Material%CDF_valence%E0(:))
      else VAL0
...
!       if (Ee < 1.1d0*Eeq) then   ! switch to nonrelativistic numerically-integrable CDF:
   ...
   case (4)  ! Non-relativistic Ritchie CDF
      VAL3:if ( (Material%Elements(j)%valent(k)) .and. (allocated(Material%CDF_valence%A)) ) then    ! Valence band
         ...
         case default  ! free electron
            call CDF_total_CS_nonrel(numpar, sigma, Se1, Ee, g_me, Zeff, Ip, Material%T_eV, Material%CDF_valence, g_me, &
                    DOS%k, DOS%Eff_m, .false., 1.0d0, Material%At_Dens, Material%DOS, numpar%CDF_model)  ! module "CS_electrons_inelastic"
         case (1)  ! Plasmon pole
            call CDF_total_CS_nonrel(numpar, sigma, Se1, Ee, g_me, Zeff, Ip, Material%T_eV, Material%CDF_valence, g_me, DOS%k, DOS%Eff_m, &
                     .false., 1.0d0, Material%At_Dens, Material%DOS, numpar%CDF_model, &
...
end subroutine get_pos_inelastic_CS


end module CS_positrons_inelastic
