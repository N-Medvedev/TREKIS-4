...
module CS_muon_elastic
use Universal_constants
use CDF_Ritchi, only: m_two_third
use Relativity
use Objects
use Read_input_data, only: m_muon_CS, m_input_folder, m_muon_elast_CS, m_folder_materials
...
subroutine get_muon_EMFP(Material, numpar, Err)
   ...
   write(*, '(a)', advance='no') ' Obtaining muon elastic scattering cross sections...'
   ...
   TRGT:do i = 1, N_targets	! for each target
      ...
      call create_energy_grid(0.01d0, Material(i)%Ph_absorption_total%E(Nsiz), Material(i)%Muon_elastic_total%E)    ! module "Little_subroutines"
      Nsiz = size(Material(i)%Muon_elastic_total%E)  ! grid for total cross section is different from core-shells grid
!       allocate(Material(i)%Muon_elastic_total%E(Nsiz))
!       Material(i)%Muon_elastic_total%E = Material(i)%Ph_absorption_total%E / grid_renorm    ! smaller grid to account for phonons
      allocate(Material(i)%Muon_elastic_total%Total(Nsiz))
      allocate(Material(i)%Muon_elastic_total%Total_MFP(Nsiz))
      Material(i)%Muon_elastic_total%Total(:) = 0.0d0
      Material(i)%Muon_elastic_total%Total_MFP(:) = 0.0d0
...
!          Model_name = 'CDF_delta_T_'//trim(adjustl(temp_c))//'K_Eeq_'//trim(adjustl(temp_c2))
         ...
         File_name = trim(adjustl(Path_total))//path_sep//trim(adjustl(m_muon_elast_CS))//'_total_'//trim(adjustl(Model_name))//'.dat'
...
!          if (file_exist) then	! just read from the file, no need to recalculate:
         if ((file_exist) .and. (.not.numpar%recalculate_MFPs)) then    ! just read from the file, no need to recalculate:
            ...
            do m = 1, Nsiz  ! for all energy grid points:
               read(FN,'(es,es,es)', IOSTAT=Reason) Material(i)%Muon_elastic_total%E(m), Material(i)%Muon_elastic_total%Total(m), Material(i)%Muon_elastic_total%Total_MFP(m)
...
!8392     continue
         if (create_file) then ! no such file => create it)
            ...
            do m = 1, Nsiz  ! for all energy grid points:
               call get_muon_elastic_CS(Material(i)%Muon_elastic_total%E(m), Material(i), Element, numpar, Material(i)%Muon_elastic_total%Total(m))   ! [A^2] below
               Material(i)%Muon_elastic_total%Total_MFP(m) = MFP_from_sigma(Material(i)%Muon_elastic_total%Total(m), Material(i)%At_Dens)    ! module "CS_general_tools"
!                Material(i)%El_inelastic_valent%Total_Se(m) = Se    ! [eV/A]
               write(FN,'(es,es,es)') Material(i)%Muon_elastic_total%E(m), Material(i)%Muon_elastic_total%Total(m), Material(i)%Muon_elastic_total%Total_MFP(m)
!                write(6,'(es,es,es)') Material(i)%Muon_elastic_total%E(m), Material(i)%Muon_elastic_total%Total(m), Material(i)%Muon_elastic_total%Total_MFP(m)
            enddo ! m = 1, Nsiz
         endif ! (file_exist)
         close(FN)
      ...
      case (2)	! Mott
         ...
         LMNT:do j =1, N_elements	! for each element
            ...
            Ngrid = size(Material(i)%Muon_elastic_total%E)
            allocate(Element%Muon_elastic%E(Ngrid))
            Element%Muon_elastic%E = Material(i)%Muon_elastic_total%E
            allocate(Element%Muon_elastic%Total(Ngrid))
            allocate(Element%Muon_elastic%Total_MFP(Ngrid))
         
            ! Calculate total elastic cross sections for this element:
            File_name = trim(adjustl(Folder_with_CS))//path_sep//trim(adjustl(m_muon_elast_CS))//'_total_'//trim(adjustl(Model_name))//'.dat'
            inquire(file=trim(adjustl(File_name)),exist=file_exist) ! check if this file is there
            create_file = .false. ! assume there is no need to create it
...
!             if (file_exist) then	! only create it if file does not exist
            if ((file_exist) .and. (.not.numpar%recalculate_MFPs)) then    ! just read from the file, no need to recalculate:
               ...
               do m = 1, Ngrid	! for all energy grid points:
                  read(FN,'(es,es,es)', IOSTAT=Reason) Element%Muon_elastic%E(m), Element%Muon_elastic%Total(m), Element%Muon_elastic%Total_MFP(m)
                  call read_file(Reason, count_lines, read_well)	! module "Dealing_with_files"
                  if (.not.read_well) then
                     close(FN)	! redo the file
                     !goto 8391
...
!8391        continue
            if (create_file) then ! no such file => create it)
               open(newunit = FN, FILE = trim(adjustl(File_name)),action='write')
               do m = 1, Ngrid	! for all energy grid points:
                  call get_muon_elastic_CS(Element%Muon_elastic%E(m), Material(i), Element, numpar, Element%Muon_elastic%Total(m))	! below
                  Element%Muon_elastic%Total_MFP(m) = MFP_from_sigma(Element%Muon_elastic%Total(m),  1.0d24)    ! module "CS_general_tools"
                  write(FN,'(es,es,es)') Element%Muon_elastic%E(m), Element%Muon_elastic%Total(m), Element%Muon_elastic%Total_MFP(m)
               enddo ! m = 1, Ngrid
            endif
            close(FN)
            ...
            Element%Muon_elastic%Total_MFP(:) = Element%Muon_elastic%Total_MFP(:) * 1.0d24/(Material(i)%At_Dens * elem_contrib)
         enddo LMNT
         ...
         do j =1, N_elements	! for each element
            ...
            do m = 1, Nsiz	! for all energy grid points:
               E => Material(i)%Muon_elastic_total%E(m)    ! photon energy [eV]
               call interpolate_data_single(Element%Muon_elastic%E,  Element%Muon_elastic%Total(:), E, sigma) ! module "Little_subroutines"
               call interpolate_data_single(Element%Muon_elastic%E,  Element%Muon_elastic%Total_MFP(:), E, MFP) ! module "Little_subroutines"
               ! Add them into the arrays:
               Material(i)%Muon_elastic_total%Total(m) = Material(i)%Muon_elastic_total%Total(m) + sigma*elem_contrib
               Material(i)%Muon_elastic_total%Total_MFP(m) = Material(i)%Muon_elastic_total%Total_MFP(m) + 1.0d0/MFP !*elem_contrib ! to be inverted
            enddo ! m
         enddo ! j
         ! Invert to get MFP:
         Material(i)%Muon_elastic_total%Total_MFP(:) = 1.0d0/Material(i)%Muon_elastic_total%Total_MFP(:) ! [A]
      ...
      case default	! exclude
         ...
         do m = 1, Nsiz ! for all energy grid points:
            Material(i)%Muon_elastic_total%Total(m) = 0.0d0
            Material(i)%Muon_elastic_total%Total_MFP(m) = 1.0d25
         enddo ! m
      end select
   ...
   write(*, '(a)') ' Done.'
!    print*, 'Muon elastic scattering cross sections are obtained.'
   
   nullify(path_sep, E, Element)
end subroutine get_muon_EMFP

 

! Interface to select the model of muon elastic scattering cross section:
subroutine get_muon_elastic_CS(Ee, Material, Element, numpar, sigma, mu_max_in, E_max, Se)
   ...
   case (1,5)     ! CDF
      ...
      if (numpar%CDF_elast_Zeff /= 1) then
         !Zeff = 1.0d0 + Equilibrium_charge_SHI(Ee, g_M_muon, Material%Mean_Z, (Material%Mean_Z-1.0d0), 0, 1.0d0) ! module "SHI_charge_state"
         Zeff = 1.0d0 + Equilibrium_charge_SHI(Ee, g_M_muon, (Material%Mean_Z-1.0d0), (Material%Mean_Z-1.0d0), 0, 1.0d0) ! module "SHI_charge_state"
      else
         Zeff = 1.0d0    ! electron charge
      endif
      
      ! Maximal phonon frequency is defined by the maximal phononic CDF peak:
      hw_ph_max = maxval( Material%CDF_phonon%E0(:) + Material%CDF_phonon%Gamma(:) ) * 10.0d0 ! Testing adjusting coefficient
      
      Mu_elastic = numpar%Mu_elastic   ! to chose the model of CS calculations below
      ...
      if (Mu_elastic == 1) then
         ...
         !call find_Wmax_equal_Wmin(g_M_muon, Material%Mean_Mass, .false., Ee, 1.0d-6, max_E0, Eeq, hw_ph_max)   ! module "CS_integration_limits"
         call find_Wmax_equal_Wmin(g_M_muon, Material%Mean_Mass, .false., Ee, 1.0d-6, max_E0, Eeq, hw_ph_max)   ! module "CS_integration_limits"
      ...
      select case (Mu_elastic)  ! chose which model for electron inelastic cross section to use
      case (1,5)  ! Delta relativistic CDF
         if (present (E_max)) then
            sigma = CDF_total_CS_delta(Mu_elastic, Ee, g_M_muon, Zeff, 1.0d-6, Material%At_Dens, Material%CDF_phonon, &
         ...
         if (present(Se)) Se = energy_loss_delta(Mu_elastic, Ee, g_M_muon, 1.0d0, Zeff, 1.0d-6, Material%At_Dens, Material%Mean_Mass, &
                                          Material%CDF_phonon, .false., 0, hw_ph_max) ! module "CDF_delta"
      case (4) ! nonrelativistic Ritchie CDF
         
         if (present (E_max)) then
            call CDF_total_CS_nonrel(numpar, sigma, Se1, Ee, g_M_muon, Zeff, 1.0d-10, Material%T_eV, Material%CDF_phonon, Material%Mean_Mass, &
                        Material%DOS%k, Material%DOS%Eff_m, .false., 0.0d0, Material%At_Dens, Material%DOS, numpar%CDF_model, &
...
!       print*, 'get_muon_elastic_CS', Ee, Eeq, Mu_elastic, sigma

   case (2)	! Mott, assume it's the same as for an muon:
...
end subroutine get_muon_elastic_CS


end module CS_muon_elastic
