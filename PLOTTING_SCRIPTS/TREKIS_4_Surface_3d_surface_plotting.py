...
in 2026
-----------------------------------
This module provides tools for parsing and visualizing 3D wire-surface output files
('OUTPUT_*Surface*.dat') generated by TREKIS simulations. It supports:

- Reading headers to extract axis labels and physical units.
- Parsing time‑dependent block data into structured 2D grids.
- Converting blocks into coordinate meshes and field arrays.
- Generating normalized MP4 animations of front and back surfaces over time.
- Batch processing of all matching surface files in a directory.

Outputs are saved as MP4 videos (front/back) alongside the input data files.
"""
