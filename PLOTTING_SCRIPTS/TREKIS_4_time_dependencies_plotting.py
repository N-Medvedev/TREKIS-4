#!/usr/bin/env python3
"""
Standalone script to batch parse and plot TREKIS 0D data files.
...
def plot_0d_file(out_folder, fname, verbose=False, xscale="auto", yscale="auto"):
    ...
    for i, (time_val, group) in enumerate(grouped):
        ...
        group_sorted = group.sort_values(by=grid_col)
        
        ax.plot(
            group_sorted[grid_col], 
            group_sorted[func_col], 
    ...
    # Place legend inside unless there are > 10 curves
    if num_groups > 10:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process and plot 0D .dat files with configurable axis scales."
    )
    parser.add_argument(
        "-d", "--dir",
        type=str,
