...
in 2026
-----------------------------------
This module provides utilities for parsing and visualizing 'OUTPUT_total*.dat'
files generated by TREKIS simulations. It supports:

- Extracting modifiers from filenames (e.g., 'above cutoff').
- Parsing headers to identify column names, units, and data.
- Mapping shorthand column labels (Nph, Ee, etc.) to descriptive names.
- Generating plots of particle numbers and energies versus time.
- Batch processing of all matching files in a directory.

Outputs are saved as PNG figures alongside the input data files.
"""
